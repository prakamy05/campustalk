...
export default function VideoPage(){
  ...
  async function createPeerAndListen(sessionId, otherUserId, amOfferer){
    ...
    const channel = supabase.channel('signals:'+sessionId)
      .on('postgres_changes', { event:'INSERT', schema:'public', table:'signals', filter:`session_id=eq.${sessionId}` },
        payload => {
        ...
        <div style={{flex:1}}>
          <div style={{background:'#0f1724', padding:12, borderRadius:10}}>
            ...
            <div style={{marginTop:12, display:'flex', gap:8, alignItems:'center'}}>
              <button onClick={startSearch} style={{padding:'10px 16px', background:'#5b21b6', borderRadius:8, border:'none'}}>
